...
# deep inside a nested data structure. In principle, there’s nothing more difficult about pulling something out from
# deep inside a nested data structure: with lists, we use [] to index or a for loop to get them them all; with
# dictionaries, we get the value associated with a particular key using [] or iterate through all the keys, accessing
# the value for each. But it’s easy to get lost in the process and think we’ve extracted something different than we
# really have. Because of this, we have created a usable technique to help we during the debugging process.
#
# Following the system described below, we will have success with extracting nested data. The process involves the
# following steps:
#
...
# The following json structure is a nested dictionary resulted from querying Twitter, asking for three tweets matching
...
# We set for ourselves the task of printing the author's username and the creation time of each tweet.
# Following the Understand-Extract-Repeat strategy, our work might look something like this:
...
# After exploring the json structure through print statements, we can remove those helping lines, leaving only the
# minimal amount of code we need for accomplishing our task.
res2 = res['statuses']
for res3 in res2:
    res4 = res3['user']
    print(res4['screen_name'], res4['created_at'])
