# Sometimes, you may need to deal with text that includes characters that are not part of the standard English alphabet,
# such as é, ö, Ф, or ¥. This is especially likely if you use REST APIs to fetch user-contributed content from social
# media sites like Twitter, Facebook, or flickr.
...
# Things only get tricky when you want to input strings into Python, or print them to an output window or write
# them to a file.

# For output, your terminal (output) window will typically be set up to display characters only from a restricted set
# of languages (perhaps just English). If you issue a print statement on a string containing other characters, it may
# not display correctly in your terminal window. Indeed, you may get an error message. We will offer a workaround later
# on this page.
...
# more “bytes” for storage in a file. We have avoided low-level details about data encodings until now, but
# understanding a little about bits and bytes will help make sense of this.
...
# distinct bytes (eight-bit sequences). There are many possible encodings. The one you will be most likely to encounter,
# using REST APIs, is called UTF-8. A single unicode character is mapped to a sequence of up to four bytes.
...
# it for you, the json.loads() function call can take care of the decoding for you. loads() takes an optional parameter,
# encoding. Its default value is ‘utf-8’, so you don’t need to specify it unless you think the text you have received
# was in some other encoding than ‘utf-8’.

# Note: You may see a u before the string in a Python 2.7 program indicating that it’s a unicode string. In Python 3,
...
# The following is a list of online-resources which may help clarify the difference between ASCII, UNICODE and UTF-8:
# https://www.youtube.com/watch?v=5aJKKgSEUnY (Understanding ASCII and Unicode (GCSE))
# https://www.youtube.com/watch?v=MijmeoH9LT4 (Characters, Symbols and the Unicode Miracle - Computerphile)
# https://www.youtube.com/watch?v=sqPTR_v4qFA (Introduction to UTF-8 and Unicode)
# https://www.gamedev.net/tutorials/_/technical/general-programming/how-about-unicode-and-utf-8-r3322/ (it's the blog
# article associated with the previous youtube video - the author's response to Servant of the Lord's comment is
